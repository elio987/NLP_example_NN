...
def trainer_model(model_name):
    """Funciton that return the flair trainer using the new corpus of the twitter dataset 
    and the structure of a pretrained model"""
    ...
    previous_tagger: SequenceTagger = SequenceTagger.load(model_name)
    ...
    return trainer,corpus,epochs
def train_model(trainer,epochs):
...
def test(model,corpus):
    """Fuction that use the model and print some predictions using the corpus."""
